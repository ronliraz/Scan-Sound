...
def in_leftside_mouth(cnt, faces):
    ...
    for (x, y, w, h) in faces:
        for c in cnt:
            if ((x+w//8) <= c[0][0] <= (x+w//2)) and (y +int(h/2) <= c[0][1] <= y+h):
                sum += 1
        #print ([x,y,w,h], cnt)
    return sum
...
def in_rightside_mouth(cnt, faces):
    ...
    for (x, y, w, h) in faces:
        for c in cnt:
            if ((x+w//2) <= c[0][0] <= (x+5*w//8)) and (y +int(h/2) <= c[0][1] <= y + h):
                sum += 1
                # print ([x,y,w,h], cnt)
    return sum
...
def main():
    ...
    cap = cv2.VideoCapture(r"C:\Users\Ron\Auto\T2MED\bad\125.mp4")
    ...
    old_img = old_img#[100:800, 100:800]
    old_img = cv2.cvtColor(old_img, cv2.COLOR_BGR2GRAY)


    while(True):
        ret1, frame = cap.read()
        frame = frame#[100:800, 100:800]
        ...
        if ret1:
            ...
            cv2.drawContours(cop, green_contours, -1, (255,100,100), 3)
            ...
            cv2.drawContours(cop, red_contours, -1, (255,105,280), 3)
            ...
            try:
                for (x, y, w, h) in faces:
                    try:
                        if (right_counter>0 and left_counter>0 and (right_counter/(left_counter + right_counter + 0.001) > 0.7 or right_counter/(left_counter + right_counter + 0.001) < 0.3)):
                            cv2.rectangle(cop, (x, y), (x + w, y + h), (10, 10, 250), 4)
                            cv2.putText(cop, 'Warning! L={}, R={}'.format(left_counter, right_counter), ((x-50)*((x-50)>0), y+int(1.1*h)) , cv2.FONT_HERSHEY_SIMPLEX , 1, (0, 0, 255) , 2, cv2.LINE_AA)
                        else:
        ...
        total_left += left_counter
        total_right += right_counter
        print("{}, {}".format(total_left, total_right))
    cap.release()
